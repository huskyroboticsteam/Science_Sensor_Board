...
/*******************************************************************************
* FUNCTION NAME:   Sensor_CAN_SendMsg
********************************************************************************
*
* Summary:
...
uint8 Sensor_CAN_SendMsg(const Sensor_CAN_TX_MSG *message) 
{
    ...
    while (retry < Sensor_CAN_RETRY_NUMBER)
    {
        shift = 1u;    /* Start from first mailbox */
        for (i = 0u; i < Sensor_CAN_NUMBER_OF_TX_MAILBOXES; i++)
        {
            /* Find Basic TX mailboxes */
            if ((Sensor_CAN_TX_MAILBOX_TYPE & shift) == 0u)
            {
                /* Find free mailbox */
                #if (CY_PSOC3 || CY_PSOC5)
                    if ((Sensor_CAN_BUF_SR_REG.byte[2] & shift) == 0u)
                #else  /* CY_PSOC4 */
                    if ((Sensor_CAN_BUF_SR_REG &
                        (uint32) ((uint32) shift << Sensor_CAN_TWO_BYTE_OFFSET)) == 0u)
                #endif /* CY_PSOC3 || CY_PSOC5 */
                    {
                        regTemp = 0u;

                        /* Set message parameters */
                        if (message->rtr != Sensor_CAN_STANDARD_MESSAGE)
                        {
                            regTemp = Sensor_CAN_TX_RTR_MASK;    /* RTR message Enable */
                        }

                        if (message->ide == Sensor_CAN_STANDARD_MESSAGE)
                        {
                            Sensor_CAN_SET_TX_ID_STANDARD_MSG(i, message->id);
                        }
                        else
                        {
                            regTemp |= Sensor_CAN_TX_IDE_MASK;
                            Sensor_CAN_SET_TX_ID_EXTENDED_MSG(i, message->id);
                        }

                        if (message->dlc < Sensor_CAN_TX_DLC_MAX_VALUE)
                        {
                            regTemp |= ((uint32) message->dlc) << Sensor_CAN_TWO_BYTE_OFFSET;
                        }
                        else
                        {
                            regTemp |= Sensor_CAN_TX_DLC_UPPER_VALUE;
                        }

                        if (message->irq != Sensor_CAN_TRANSMIT_INT_DISABLE)
                        {
                            regTemp |= Sensor_CAN_TX_INT_ENABLE_MASK;    /* Transmit Interrupt Enable */
                        }

                        for (j = 0u; (j < message->dlc) && (j < Sensor_CAN_TX_DLC_MAX_VALUE); j++)
                        {
                            #if (CY_PSOC3 || CY_PSOC5)
                                Sensor_CAN_TX_DATA_BYTE(i, j) = message->msg->byte[j];
                            #else /* CY_PSOC4 */
                                Sensor_CAN_TX_DATA_BYTE(i, j, message->msg->byte[j]);
                            #endif /* CY_PSOC3 || CY_PSOC5 */
                        }
    ...
    CyIntDisable(Sensor_CAN_ISR_NUMBER);
                        ...
                        CY_SET_REG32(Sensor_CAN_TX_CMD_PTR(i), (regTemp | Sensor_CAN_TX_WPN_SET));

                        #if (CY_PSOC3 || CY_PSOC5)
                            CY_SET_REG32(Sensor_CAN_TX_CMD_PTR(i), Sensor_CAN_SEND_MESSAGE);
                        #else /* CY_PSOC4 */
                            if (message->sst != Sensor_CAN_STANDARD_MESSAGE)
                            {
                                /* Single Shot Transmission */
                                Sensor_CAN_TX_CMD_REG(i) |= Sensor_CAN_SEND_MESSAGE |
                                Sensor_CAN_TX_ABORT_MASK;
                            }
                            else
                            {
                                Sensor_CAN_TX_CMD_REG(i) |= Sensor_CAN_SEND_MESSAGE;
...
/*******************************************************************************
* FUNCTION NAME:   Sensor_CAN_TxCancel
********************************************************************************
*
* Summary:
...
{
    if (bufferId < Sensor_CAN_NUMBER_OF_TX_MAILBOXES)
    {
        Sensor_CAN_TX_ABORT_MESSAGE(bufferId);
    }
}
    ...
    /*******************************************************************************
    * FUNCTION NAME:   Sensor_CAN_SendMsg0
    ********************************************************************************
    *
    * Summary:
    ...
    *******************************************************************************/
    uint8 Sensor_CAN_SendMsg0(void) 
    {
        uint8 result = CYRET_SUCCESS;
        ...
        #if (CY_PSOC3 || CY_PSOC5)
            if ((Sensor_CAN_TX[0u].txcmd.byte[0u] & Sensor_CAN_TX_REQUEST_PENDING) != 0u)
        #else  /* CY_PSOC4 */
            if ((Sensor_CAN_TX_CMD_REG(0u) & Sensor_CAN_TX_REQUEST_PENDING) != 0u)
        #endif /* CY_PSOC3 || CY_PSOC5 */
            {
            ...
            else
            {
                ...
                #ifdef Sensor_CAN_SEND_MSG_0_CALLBACK
                    Sensor_CAN_SendMsg_0_Callback();
                #endif /* Sensor_CAN_SEND_MSG_0_CALLBACK */

                CY_SET_REG32(Sensor_CAN_TX_CMD_PTR(0u),
                CY_GET_REG32(Sensor_CAN_TX_CMD_PTR(0u)) | Sensor_CAN_SEND_MESSAGE);
            }
    ...
    /*******************************************************************************
    * FUNCTION NAME:   Sensor_CAN_SendMsg1
    ********************************************************************************
    *
    * Summary:
    ...
    *******************************************************************************/
    uint8 Sensor_CAN_SendMsg1(void) 
    {
        uint8 result = CYRET_SUCCESS;
        ...
        #if (CY_PSOC3 || CY_PSOC5)
            if ((Sensor_CAN_TX[1u].txcmd.byte[0u] & Sensor_CAN_TX_REQUEST_PENDING) != 0u)
        #else  /* CY_PSOC4 */
            if ((Sensor_CAN_TX_CMD_REG(1u) & Sensor_CAN_TX_REQUEST_PENDING) != 0u)
        #endif /* CY_PSOC3 || CY_PSOC5 */
            {
                result = Sensor_CAN_FAIL;
            }
            else
            {
                ...
                #ifdef Sensor_CAN_SEND_MSG_1_CALLBACK
                    Sensor_CAN_SendMsg_1_Callback();
                #endif /* Sensor_CAN_SEND_MSG_1_CALLBACK */

                CY_SET_REG32(Sensor_CAN_TX_CMD_PTR(1u),
                CY_GET_REG32(Sensor_CAN_TX_CMD_PTR(1u)) | Sensor_CAN_SEND_MESSAGE);
            }
    ...
    /*******************************************************************************
    * FUNCTION NAME:   Sensor_CAN_SendMsg2
    ********************************************************************************
    *
    * Summary:
    ...
    *******************************************************************************/
    uint8 Sensor_CAN_SendMsg2(void) 
    {
        uint8 result = CYRET_SUCCESS;
        ...
        #if (CY_PSOC3 || CY_PSOC5)
            if ((Sensor_CAN_TX[2u].txcmd.byte[0u] & Sensor_CAN_TX_REQUEST_PENDING) != 0u)
        #else  /* CY_PSOC4 */
            if ((Sensor_CAN_TX_CMD_REG(2u) & Sensor_CAN_TX_REQUEST_PENDING) != 0u)
        #endif /* CY_PSOC3 || CY_PSOC5 */
            {
                result = Sensor_CAN_FAIL;
            }
            else
            {
                ...
                #ifdef Sensor_CAN_SEND_MSG_2_CALLBACK
                    Sensor_CAN_SendMsg_2_Callback();
                #endif /* Sensor_CAN_SEND_MSG_2_CALLBACK */

                CY_SET_REG32(Sensor_CAN_TX_CMD_PTR(2u),
                CY_GET_REG32(Sensor_CAN_TX_CMD_PTR(2u)) | Sensor_CAN_SEND_MESSAGE);
            }
    ...
    /*******************************************************************************
    * FUNCTION NAME:   Sensor_CAN_SendMsg3
    ********************************************************************************
    *
    * Summary:
    ...
    *******************************************************************************/
    uint8 Sensor_CAN_SendMsg3(void) 
    {
        uint8 result = CYRET_SUCCESS;
        ...
        #if (CY_PSOC3 || CY_PSOC5)
            if ((Sensor_CAN_TX[3u].txcmd.byte[0u] & Sensor_CAN_TX_REQUEST_PENDING) != 0u)
        #else  /* CY_PSOC4 */
            if ((Sensor_CAN_TX_CMD_REG(3u) & Sensor_CAN_TX_REQUEST_PENDING) != 0u)
        #endif /* CY_PSOC3 || CY_PSOC5 */
            {
                result = Sensor_CAN_FAIL;
            }
            else
            {
                ...
                #ifdef Sensor_CAN_SEND_MSG_3_CALLBACK
                    Sensor_CAN_SendMsg_3_Callback();
                #endif /* Sensor_CAN_SEND_MSG_3_CALLBACK */

                CY_SET_REG32(Sensor_CAN_TX_CMD_PTR(3u),
                CY_GET_REG32(Sensor_CAN_TX_CMD_PTR(3u)) | Sensor_CAN_SEND_MESSAGE);
            }
    ...
    /*******************************************************************************
    * FUNCTION NAME:   Sensor_CAN_SendMsg4
    ********************************************************************************
    *
    * Summary:
    ...
    *******************************************************************************/
    uint8 Sensor_CAN_SendMsg4(void) 
    {
        uint8 result = CYRET_SUCCESS;
        ...
        #if (CY_PSOC3 || CY_PSOC5)
            if ((Sensor_CAN_TX[4u].txcmd.byte[0u] & Sensor_CAN_TX_REQUEST_PENDING) != 0u)
        #else  /* CY_PSOC4 */
            if ((Sensor_CAN_TX_CMD_REG(4u) & Sensor_CAN_TX_REQUEST_PENDING) != 0u)
        #endif /* CY_PSOC3 || CY_PSOC5 */
            {
                result = Sensor_CAN_FAIL;
            }
            else
            {
                ...
                #ifdef Sensor_CAN_SEND_MSG_4_CALLBACK
                    Sensor_CAN_SendMsg_4_Callback();
                #endif /* Sensor_CAN_SEND_MSG_4_CALLBACK */

                CY_SET_REG32(Sensor_CAN_TX_CMD_PTR(4u),
                CY_GET_REG32(Sensor_CAN_TX_CMD_PTR(4u)) | Sensor_CAN_SEND_MESSAGE);
            }
    ...
    /*******************************************************************************
    * FUNCTION NAME:   Sensor_CAN_SendMsg5
    ********************************************************************************
    *
    * Summary:
    ...
    *******************************************************************************/
    uint8 Sensor_CAN_SendMsg5(void) 
    {
        uint8 result = CYRET_SUCCESS;
        ...
        #if (CY_PSOC3 || CY_PSOC5)
            if ((Sensor_CAN_TX[5u].txcmd.byte[0u] & Sensor_CAN_TX_REQUEST_PENDING) != 0u)
        #else  /* CY_PSOC4 */
            if ((Sensor_CAN_TX_CMD_REG(5u) & Sensor_CAN_TX_REQUEST_PENDING) != 0u)
        #endif /* CY_PSOC3 || CY_PSOC5 */
            {
                result = Sensor_CAN_FAIL;
            }
            else
            {
                ...
                #ifdef Sensor_CAN_SEND_MSG_5_CALLBACK
                    Sensor_CAN_SendMsg_5_Callback();
                #endif /* Sensor_CAN_SEND_MSG_5_CALLBACK */

                CY_SET_REG32(Sensor_CAN_TX_CMD_PTR(5u),
                CY_GET_REG32(Sensor_CAN_TX_CMD_PTR(5u)) | Sensor_CAN_SEND_MESSAGE);
            }
    ...
    /*******************************************************************************
    * FUNCTION NAME:   Sensor_CAN_SendMsg6
    ********************************************************************************
    *
    * Summary:
    ...
    *******************************************************************************/
    uint8 Sensor_CAN_SendMsg6(void) 
    {
        uint8 result = CYRET_SUCCESS;
        ...
        #if (CY_PSOC3 || CY_PSOC5)
            if ((Sensor_CAN_TX[6u].txcmd.byte[0u] & Sensor_CAN_TX_REQUEST_PENDING) != 0u)
        #else  /* CY_PSOC4 */
            if ((Sensor_CAN_TX_CMD_REG(6u) & Sensor_CAN_TX_REQUEST_PENDING) != 0u)
        #endif /* CY_PSOC3 || CY_PSOC5 */
            {
                result = Sensor_CAN_FAIL;
            }
            else
            {
                ...
                #ifdef Sensor_CAN_SEND_MSG_6_CALLBACK
                    Sensor_CAN_SendMsg_6_Callback();
                #endif /* Sensor_CAN_SEND_MSG_6_CALLBACK */

                CY_SET_REG32(Sensor_CAN_TX_CMD_PTR(6u),
                CY_GET_REG32(Sensor_CAN_TX_CMD_PTR(6u)) | Sensor_CAN_SEND_MESSAGE);
            }
    ...
    /*******************************************************************************
    * FUNCTION NAME:   Sensor_CAN_SendMsg7)
    ********************************************************************************
    *
    * Summary:
    ...
    *******************************************************************************/
    uint8 Sensor_CAN_SendMsg7(void) 
    {
        uint8 result = CYRET_SUCCESS;
        ...
        #if (CY_PSOC3 || CY_PSOC5)
            if ((Sensor_CAN_TX[7u].txcmd.byte[0u] & Sensor_CAN_TX_REQUEST_PENDING) != 0u)
        #else  /* CY_PSOC4 */
            if ((Sensor_CAN_TX_CMD_REG(7u) & Sensor_CAN_TX_REQUEST_PENDING) != 0u)
        #endif /* CY_PSOC3 || CY_PSOC5 */
            {
                result = Sensor_CAN_FAIL;
            }
            else
            {
                ...
                #ifdef Sensor_CAN_SEND_MSG_7_CALLBACK
                    Sensor_CAN_SendMsg_7_Callback();
                #endif /* Sensor_CAN_SEND_MSG_7_CALLBACK */

                CY_SET_REG32(Sensor_CAN_TX_CMD_PTR(7u),
                CY_GET_REG32(Sensor_CAN_TX_CMD_PTR(7u)) | Sensor_CAN_SEND_MESSAGE);
            }
...
/*******************************************************************************
* FUNCTION NAME:   Sensor_CAN_ReceiveMsg
********************************************************************************
*
* Summary:
...
{
    #if (CY_PSOC3 || CY_PSOC5)
        if ((Sensor_CAN_RX[rxMailbox].rxcmd.byte[0u] & Sensor_CAN_RX_ACK_MSG) != 0u)
    #else  /* CY_PSOC4 */
        if ((Sensor_CAN_RX_CMD_REG(rxMailbox) & Sensor_CAN_RX_ACK_MSG) != 0u)
    #endif /* CY_PSOC3 || CY_PSOC5 */
        {
            ...
            #ifdef Sensor_CAN_RECEIVE_MSG_CALLBACK
                Sensor_CAN_ReceiveMsg_Callback();
            #endif /* Sensor_CAN_RECEIVE_MSG_CALLBACK */

            #if (CY_PSOC3 || CY_PSOC5)
                Sensor_CAN_RX[rxMailbox].rxcmd.byte[0u] |= Sensor_CAN_RX_ACK_MSG;
            #else  /* CY_PSOC4 */
                Sensor_CAN_RX_CMD_REG(rxMailbox) |= Sensor_CAN_RX_ACK_MSG;
            #endif /* CY_PSOC3 || CY_PSOC5 */
        }
}
    ...
    /*******************************************************************************
    * FUNCTION NAME:   Sensor_CAN_ReceiveMsg0
    ********************************************************************************
    *
    * Summary:
    ...
    *******************************************************************************/
    void Sensor_CAN_ReceiveMsg0(void) 
    {
        ...
        #ifdef Sensor_CAN_RECEIVE_MSG_0_CALLBACK
            Sensor_CAN_ReceiveMsg_0_Callback();
        #endif /* Sensor_CAN_RECEIVE_MSG_0_CALLBACK */

        Sensor_CAN_RX_ACK_MESSAGE(0u);
    }
    ...
    /*******************************************************************************
    * FUNCTION NAME:    Sensor_CAN_ReceiveMsg1
    ********************************************************************************
    *
    * Summary:
    ...
    *******************************************************************************/
    void Sensor_CAN_ReceiveMsg1(void) 
    {
        ...
        #ifdef Sensor_CAN_RECEIVE_MSG_1_CALLBACK
            Sensor_CAN_ReceiveMsg_1_Callback();
        #endif /* Sensor_CAN_RECEIVE_MSG_1_CALLBACK */

        Sensor_CAN_RX_ACK_MESSAGE(1u);
    }
    ...
    /*******************************************************************************
    * FUNCTION NAME:   Sensor_CAN_ReceiveMsg2
    ********************************************************************************
    *
    * Summary:
    ...
    *******************************************************************************/
    void Sensor_CAN_ReceiveMsg2(void) 
    {
        ...
        #ifdef Sensor_CAN_RECEIVE_MSG_2_CALLBACK
            Sensor_CAN_ReceiveMsg_2_Callback();
        #endif /* Sensor_CAN_RECEIVE_MSG_2_CALLBACK */

        Sensor_CAN_RX_ACK_MESSAGE(2u);
    }
    ...
    /*******************************************************************************
    * FUNCTION NAME:   Sensor_CAN_ReceiveMsg3
    ********************************************************************************
    *
    * Summary:
    ...
    *******************************************************************************/
    void Sensor_CAN_ReceiveMsg3(void) 
    {
        ...
        #ifdef Sensor_CAN_RECEIVE_MSG_3_CALLBACK
            Sensor_CAN_ReceiveMsg_3_Callback();
        #endif /* Sensor_CAN_RECEIVE_MSG_3_CALLBACK */

        Sensor_CAN_RX_ACK_MESSAGE(3u);
    }
    ...
    /*******************************************************************************
    * FUNCTION NAME:   Sensor_CAN_ReceiveMsg4
    ********************************************************************************
    *
    * Summary:
    ...
    *******************************************************************************/
    void Sensor_CAN_ReceiveMsg4(void) 
    {
        ...
        #ifdef Sensor_CAN_RECEIVE_MSG_4_CALLBACK
            Sensor_CAN_ReceiveMsg_4_Callback();
        #endif /* Sensor_CAN_RECEIVE_MSG_4_CALLBACK */

        Sensor_CAN_RX_ACK_MESSAGE(4u);
    }
    ...
    /*******************************************************************************
    * FUNCTION NAME:   Sensor_CAN_ReceiveMsg5
    ********************************************************************************
    *
    * Summary:
    ...
    *******************************************************************************/
    void Sensor_CAN_ReceiveMsg5(void) 
    {
        ...
        #ifdef Sensor_CAN_RECEIVE_MSG_5_CALLBACK
            Sensor_CAN_ReceiveMsg_5_Callback();
        #endif /* Sensor_CAN_RECEIVE_MSG_5_CALLBACK */

        Sensor_CAN_RX_ACK_MESSAGE(5u);
    }
    ...
    /*******************************************************************************
    * FUNCTION NAME:   Sensor_CAN_ReceiveMsg6
    ********************************************************************************
    *
    * Summary:
    ...
    *******************************************************************************/
    void Sensor_CAN_ReceiveMsg6(void) 
    {
        ...
        #ifdef Sensor_CAN_RECEIVE_MSG_6_CALLBACK
            Sensor_CAN_ReceiveMsg_6_Callback();
        #endif /* Sensor_CAN_RECEIVE_MSG_6_CALLBACK */

        Sensor_CAN_RX_ACK_MESSAGE(6u);
    }
    ...
    /*******************************************************************************
    * FUNCTION NAME:   Sensor_CAN_ReceiveMsg7
    ********************************************************************************
    *
    * Summary:
    ...
    *******************************************************************************/
    void Sensor_CAN_ReceiveMsg7(void) 
    {
        ...
        #ifdef Sensor_CAN_RECEIVE_MSG_7_CALLBACK
            Sensor_CAN_ReceiveMsg_7_Callback();
        #endif /* Sensor_CAN_RECEIVE_MSG_7_CALLBACK */

        Sensor_CAN_RX_ACK_MESSAGE(7u);
    }
    ...
    /*******************************************************************************
    * FUNCTION NAME:   Sensor_CAN_ReceiveMsg8
    ********************************************************************************
    *
    * Summary:
    ...
    *******************************************************************************/
    void Sensor_CAN_ReceiveMsg8(void) 
    {
        ...
        #ifdef Sensor_CAN_RECEIVE_MSG_8_CALLBACK
            Sensor_CAN_ReceiveMsg_8_Callback();
        #endif /* Sensor_CAN_RECEIVE_MSG_8_CALLBACK */

        Sensor_CAN_RX_ACK_MESSAGE(8u);
    }
    ...
    /*******************************************************************************
    * FUNCTION NAME:   Sensor_CAN_ReceiveMsg9
    ********************************************************************************
    *
    * Summary:
    ...
    *******************************************************************************/
    void Sensor_CAN_ReceiveMsg9(void) 
    {
        ...
        #ifdef Sensor_CAN_RECEIVE_MSG_9_CALLBACK
            Sensor_CAN_ReceiveMsg_9_Callback();
        #endif /* Sensor_CAN_RECEIVE_MSG_9_CALLBACK */

        Sensor_CAN_RX_ACK_MESSAGE(9u);
    }
    ...
    /*******************************************************************************
    * FUNCTION NAME:   Sensor_CAN_ReceiveMsg10
    ********************************************************************************
    *
    * Summary:
    ...
    *******************************************************************************/
    void Sensor_CAN_ReceiveMsg10(void) 
    {
        ...
        #ifdef Sensor_CAN_RECEIVE_MSG_10_CALLBACK
            Sensor_CAN_ReceiveMsg_10_Callback();
        #endif /* Sensor_CAN_RECEIVE_MSG_10_CALLBACK */

        Sensor_CAN_RX_ACK_MESSAGE(10u);
    }
    ...
    /*******************************************************************************
    * FUNCTION NAME:   Sensor_CAN_ReceiveMsg11
    ********************************************************************************
    *
    * Summary:
    ...
    *******************************************************************************/
    void Sensor_CAN_ReceiveMsg11(void) 
    {
        ...
        #ifdef Sensor_CAN_RECEIVE_MSG_11_CALLBACK
            Sensor_CAN_ReceiveMsg_11_Callback();
        #endif /* Sensor_CAN_RECEIVE_MSG_11_CALLBACK */

        Sensor_CAN_RX_ACK_MESSAGE(11u);
    }
    ...
    /*******************************************************************************
    * FUNCTION NAME:   Sensor_CAN_ReceiveMsg12
    ********************************************************************************
    *
    * Summary:
    ...
    *******************************************************************************/
    void Sensor_CAN_ReceiveMsg12(void) 
    {
        ...
        #ifdef Sensor_CAN_RECEIVE_MSG_12_CALLBACK
            Sensor_CAN_ReceiveMsg_12_Callback();
        #endif /* Sensor_CAN_RECEIVE_MSG_12_CALLBACK */

        Sensor_CAN_RX_ACK_MESSAGE(12u);
    }
    ...
    /*******************************************************************************
    * FUNCTION NAME:   Sensor_CAN_ReceiveMsg13
    ********************************************************************************
    *
    * Summary:
    ...
    *******************************************************************************/
    void Sensor_CAN_ReceiveMsg13(void) 
    {
        ...
        #ifdef Sensor_CAN_RECEIVE_MSG_13_CALLBACK
            Sensor_CAN_ReceiveMsg_13_Callback();
        #endif /* Sensor_CAN_RECEIVE_MSG_13_CALLBACK */

        Sensor_CAN_RX_ACK_MESSAGE(13u);
    }
    ...
    /*******************************************************************************
    * FUNCTION NAME:   Sensor_CAN_ReceiveMsg14
    ********************************************************************************
    *
    * Summary:
    ...
    *******************************************************************************/
    void Sensor_CAN_ReceiveMsg14(void) 
    {
        ...
        #ifdef Sensor_CAN_RECEIVE_MSG_14_CALLBACK
            Sensor_CAN_ReceiveMsg_14_Callback();
        #endif /* Sensor_CAN_RECEIVE_MSG_14_CALLBACK */

        Sensor_CAN_RX_ACK_MESSAGE(14u);
    }
    ...
    /*******************************************************************************
    * FUNCTION NAME:   Sensor_CAN_ReceiveMsg15
    ********************************************************************************
    *
    * Summary:
    ...
    *******************************************************************************/
    void Sensor_CAN_ReceiveMsg15(void) 
    {
        ...
        #ifdef Sensor_CAN_RECEIVE_MSG_15_CALLBACK
            Sensor_CAN_ReceiveMsg_15_Callback();
        #endif /* Sensor_CAN_RECEIVE_MSG_15_CALLBACK */

        Sensor_CAN_RX_ACK_MESSAGE(15u);
    }
