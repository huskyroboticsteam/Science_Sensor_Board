...
/*******************************************************************************
* Function Name: HumTemp_I2C_I2C_ISR
****************************************************************************//**
*
*  Handles the Interrupt Service Routine for the SCB I2C mode.
*
*******************************************************************************/
CY_ISR(HumTemp_I2C_I2C_ISR)
{
    uint32 diffCount;
    uint32 endTransfer;

#ifdef HumTemp_I2C_I2C_ISR_ENTRY_CALLBACK
    HumTemp_I2C_I2C_ISR_EntryCallback();
#endif /* HumTemp_I2C_I2C_ISR_ENTRY_CALLBACK */

#if (HumTemp_I2C_I2C_CUSTOM_ADDRESS_HANDLER_CONST)
    uint32 response;

    response = HumTemp_I2C_I2C_ACK_ADDR;
    ...
    if(NULL != HumTemp_I2C_customIntrHandler)
    {
        HumTemp_I2C_customIntrHandler();
    }

    if(HumTemp_I2C_CHECK_INTR_I2C_EC_MASKED(HumTemp_I2C_INTR_I2C_EC_WAKE_UP))
    {
    ...
    */
    if(HumTemp_I2C_CHECK_I2C_FSM_MASTER)
    {
        #if(HumTemp_I2C_I2C_MASTER)
        {
            ...
            */
            if(HumTemp_I2C_CHECK_INTR_MASTER_MASKED(HumTemp_I2C_INTR_MASTER_I2C_BUS_ERROR))
            {
                HumTemp_I2C_mstrStatus |= (uint16) (HumTemp_I2C_I2C_MSTAT_ERR_XFER |
                                                         HumTemp_I2C_I2C_MSTAT_ERR_BUS_ERROR);

                endTransfer = HumTemp_I2C_I2C_CMPLT_ANY_TRANSFER;
            }
            ...
            */
            if(HumTemp_I2C_CHECK_INTR_MASTER_MASKED(HumTemp_I2C_INTR_MASTER_I2C_ARB_LOST))
            {
                HumTemp_I2C_mstrStatus |= (uint16) (HumTemp_I2C_I2C_MSTAT_ERR_XFER |
                                                         HumTemp_I2C_I2C_MSTAT_ERR_ARB_LOST);

                endTransfer = HumTemp_I2C_I2C_CMPLT_ANY_TRANSFER;
            }

            #if(HumTemp_I2C_I2C_MULTI_MASTER_SLAVE)
            {
                /* I2C_MASTER_CMD_M_START_ON_IDLE:
                * MultiMaster-Slave does not generate start, because Slave was addressed.
                * Pass control to slave.
                */
                if(HumTemp_I2C_CHECK_I2C_MASTER_CMD(HumTemp_I2C_I2C_MASTER_CMD_M_START_ON_IDLE))
                {
                    HumTemp_I2C_mstrStatus |= (uint16) (HumTemp_I2C_I2C_MSTAT_ERR_XFER |
                                                             HumTemp_I2C_I2C_MSTAT_ERR_ABORT_XFER);
            ...
            */
            if(0u != endTransfer)
            {
                ...
                HumTemp_I2C_mstrStatus |= (uint16) HumTemp_I2C_GET_I2C_MSTAT_CMPLT;
                ...
                #if(HumTemp_I2C_I2C_MULTI_MASTER_SLAVE)
                {
                    if(HumTemp_I2C_CHECK_I2C_FSM_ADDR)
                    {
                        ...
                        */
                        if(HumTemp_I2C_CHECK_I2C_MASTER_CMD(HumTemp_I2C_I2C_MASTER_CMD_M_START_ON_IDLE))
                        {
                            HumTemp_I2C_I2C_MASTER_CLEAR_START;

                            endTransfer = HumTemp_I2C_I2C_CMPLT_ANY_TRANSFER; /* Pass control to Slave */
                        }
                        ...
                        */
                        else if((!HumTemp_I2C_CHECK_INTR_SLAVE_MASKED(HumTemp_I2C_INTR_SLAVE_I2C_BUS_ERROR))
                               && HumTemp_I2C_CHECK_INTR_MASTER_MASKED(HumTemp_I2C_INTR_MASTER_I2C_ARB_LOST))
                        {
                            endTransfer = HumTemp_I2C_I2C_CMPLT_ANY_TRANSFER; /* Pass control to Slave */
                        }
                        else
                        ...
                        {
                            HumTemp_I2C_CLEAR_TX_FIFO; /* Shifter keeps address, clear it */

                            HumTemp_I2C_DISABLE_MASTER_AUTO_DATA_ACK; /* In case of reading disable autoACK */
                        ...
                        else
                        {
                            ...
                            HumTemp_I2C_state = HumTemp_I2C_I2C_FSM_EXIT_IDLE;
                        }
                    }
                    else
                    {
    ...
    else /* (HumTemp_I2C_CHECK_I2C_FSM_SLAVE) */
    {
        #if(HumTemp_I2C_I2C_SLAVE)
        {
            /* INTR_SLAVE_I2C_BUS_ERROR or HumTemp_I2C_INTR_SLAVE_I2C_ARB_LOST:
            * A Misplaced Start or Stop condition occurred on the bus: set a flag
            * to notify an error condition.
            */
            if(HumTemp_I2C_CHECK_INTR_SLAVE_MASKED(HumTemp_I2C_INTR_SLAVE_I2C_BUS_ERROR |
                                                        HumTemp_I2C_INTR_SLAVE_I2C_ARB_LOST))
            {
                if(HumTemp_I2C_CHECK_I2C_FSM_RD)
                {
                    /* TX direction: master reads from slave */
                    HumTemp_I2C_slStatus &= (uint8) ~HumTemp_I2C_I2C_SSTAT_RD_BUSY;
                    HumTemp_I2C_slStatus |= (uint8) (HumTemp_I2C_I2C_SSTAT_RD_ERR |
                                                          HumTemp_I2C_I2C_SSTAT_RD_CMPLT);
                }
                else
                {
                    /* RX direction: master writes into slave */
                    HumTemp_I2C_slStatus &= (uint8) ~HumTemp_I2C_I2C_SSTAT_WR_BUSY;
                    HumTemp_I2C_slStatus |= (uint8) (HumTemp_I2C_I2C_SSTAT_WR_ERR |
                                                          HumTemp_I2C_I2C_SSTAT_WR_CMPLT);
                }
    ...
    */
    ...
    if(HumTemp_I2C_CHECK_I2C_FSM_MASTER)
    {
        #if(HumTemp_I2C_I2C_MASTER)
        {
            ...
            */
            if(HumTemp_I2C_CHECK_INTR_MASTER_MASKED(HumTemp_I2C_INTR_MASTER_I2C_STOP))
            {
                HumTemp_I2C_ClearMasterInterruptSource(HumTemp_I2C_INTR_MASTER_I2C_STOP);
            ...
            {
                if(HumTemp_I2C_CHECK_I2C_FSM_ADDR) /* Address stage */
                {
                    /* INTR_MASTER_I2C_NACK:
                    * The master sent an address but it was NACKed by the slave. Complete transaction.
                    */
                    if(HumTemp_I2C_CHECK_INTR_MASTER_MASKED(HumTemp_I2C_INTR_MASTER_I2C_NACK))
                    {
                        HumTemp_I2C_ClearMasterInterruptSource(HumTemp_I2C_INTR_MASTER_I2C_NACK);

                        HumTemp_I2C_mstrStatus |= (uint16) (HumTemp_I2C_I2C_MSTAT_ERR_XFER |
                                                                 HumTemp_I2C_I2C_MSTAT_ERR_ADDR_NAK);
                    ...
                    */
                    ...
                    {
                        if(HumTemp_I2C_CHECK_I2C_FSM_RD) /* Reading */
                        {
                        ...
                        else /* Writing */
                        {
                            ...
                            if(0u != HumTemp_I2C_mstrWrBufSize)
                            {
                                /* Enable INTR.TX_EMPTY if there is data to transmit */
                                HumTemp_I2C_SetTxInterruptMode(HumTemp_I2C_INTR_TX_EMPTY);
                            }
                        }
                    }
                }

                if(HumTemp_I2C_CHECK_I2C_FSM_DATA) /* Data phase */
                {
                    if(HumTemp_I2C_CHECK_I2C_FSM_RD) /* Reading */
                    {
                        /* INTR_RX_FULL:
                        ...
                        */
                        if(HumTemp_I2C_CHECK_INTR_RX_MASKED(HumTemp_I2C_INTR_RX_FULL))
                        {
                            /* Calculate difference */
                            diffCount =  HumTemp_I2C_mstrRdBufSize -
                                        (HumTemp_I2C_mstrRdBufIndex + HumTemp_I2C_GET_RX_FIFO_ENTRIES);

                            /* Proceed transaction or end it when RX FIFO becomes FULL again */
                            if(diffCount > HumTemp_I2C_I2C_FIFO_SIZE)
                            {
                                diffCount = HumTemp_I2C_I2C_FIFO_SIZE;
                            }
                            else
                            {
                                if(0u == diffCount)
                                {
                                    HumTemp_I2C_DISABLE_MASTER_AUTO_DATA_ACK;
                            ...
                            {
                                HumTemp_I2C_mstrRdBufPtr[HumTemp_I2C_mstrRdBufIndex] = (uint8)
                                                                                        HumTemp_I2C_RX_FIFO_RD_REG;
                                HumTemp_I2C_mstrRdBufIndex++;
                            }
                        }
                        /* INTR_RX_NOT_EMPTY:
                        ...
                        */
                        else if(HumTemp_I2C_CHECK_INTR_RX_MASKED(HumTemp_I2C_INTR_RX_NOT_EMPTY))
                        {
                            /* Put data in component buffer */
                            HumTemp_I2C_mstrRdBufPtr[HumTemp_I2C_mstrRdBufIndex] = (uint8) HumTemp_I2C_RX_FIFO_RD_REG;
                            HumTemp_I2C_mstrRdBufIndex++;

                            if(HumTemp_I2C_mstrRdBufIndex < HumTemp_I2C_mstrRdBufSize)
                            {
                    ...
                    else /* Writing */
                    {
                        ...
                        */
                        if(HumTemp_I2C_CHECK_INTR_MASTER_MASKED(HumTemp_I2C_INTR_MASTER_I2C_NACK))
                        {
                            ...
                            HumTemp_I2C_mstrWrBufIndexTmp -= (HumTemp_I2C_GET_TX_FIFO_ENTRIES +
                                                                   HumTemp_I2C_GET_TX_FIFO_SR_VALID);

                            /* Update number of transferred bytes */
                            HumTemp_I2C_mstrWrBufIndex = HumTemp_I2C_mstrWrBufIndexTmp;

                            HumTemp_I2C_mstrStatus |= (uint16) (HumTemp_I2C_I2C_MSTAT_ERR_XFER |
                                                                     HumTemp_I2C_I2C_MSTAT_ERR_SHORT_XFER);

                            HumTemp_I2C_CLEAR_TX_FIFO;
                        ...
                        */
                        else if(HumTemp_I2C_CHECK_INTR_TX_MASKED(HumTemp_I2C_INTR_TX_EMPTY))
                        {
                            while(HumTemp_I2C_I2C_FIFO_SIZE != HumTemp_I2C_GET_TX_FIFO_ENTRIES)
                            {
                                ...
                                */
                                if(HumTemp_I2C_mstrWrBufIndexTmp < HumTemp_I2C_mstrWrBufSize)
                                {
                                    ...
                                    */
                                    if(HumTemp_I2C_mstrWrBufIndexTmp == (HumTemp_I2C_mstrWrBufSize - 1u))
                                    {
                                    ...
                                    HumTemp_I2C_TX_FIFO_WR_REG = (uint32) HumTemp_I2C_mstrWrBufPtr[HumTemp_I2C_mstrWrBufIndexTmp];
                                    HumTemp_I2C_mstrWrBufIndexTmp++;
                                }
                                else
                                {
                        ...
                        #else
                            HumTemp_I2C_ClearTxInterruptSource(HumTemp_I2C_INTR_TX_EMPTY);
                        #endif /* (HumTemp_I2C_CY_SCBIP_V0) */
                        }
                        /* INTR_TX_UNDERFLOW:
                        ...
                        */
                        else if(HumTemp_I2C_CHECK_INTR_TX_MASKED(HumTemp_I2C_INTR_TX_UNDERFLOW))
                        {
                            /* Update number of transferred bytes */
                            HumTemp_I2C_mstrWrBufIndex = HumTemp_I2C_mstrWrBufIndexTmp;
                ...
                if(0u != endTransfer) /* Complete transfer */
                {
                    ...
                    if(HumTemp_I2C_CHECK_I2C_MODE_NO_STOP(HumTemp_I2C_mstrControl))
                    {
                        ...
                        HumTemp_I2C_mstrStatus |= (uint16) (HumTemp_I2C_I2C_MSTAT_XFER_HALT |
                                                                 HumTemp_I2C_GET_I2C_MSTAT_CMPLT);

                        HumTemp_I2C_state = HumTemp_I2C_I2C_FSM_MSTR_HALT;
                    }
                    else
                    {
        ...
        } /* (HumTemp_I2C_I2C_MASTER) */
        #endif

    } /* (HumTemp_I2C_CHECK_I2C_FSM_MASTER) */


    /* FSM Slave */
    else if(HumTemp_I2C_CHECK_I2C_FSM_SLAVE)
    {
        #if(HumTemp_I2C_I2C_SLAVE)
        {
            /* INTR_SLAVE_NACK:
            ...
            */
            if(HumTemp_I2C_CHECK_INTR_SLAVE_MASKED(HumTemp_I2C_INTR_SLAVE_I2C_NACK))
            {
                HumTemp_I2C_ClearSlaveInterruptSource(HumTemp_I2C_INTR_SLAVE_I2C_NACK);

                /* All entries that remain in TX FIFO are: FIFO Size + 1 (SHIFTER) */
                diffCount = (HumTemp_I2C_GET_TX_FIFO_ENTRIES + HumTemp_I2C_GET_TX_FIFO_SR_VALID);

                if(HumTemp_I2C_slOverFlowCount > diffCount) /* Overflow */
                {
                    HumTemp_I2C_slStatus |= (uint8) HumTemp_I2C_I2C_SSTAT_RD_OVFL;
                }
                else /* No Overflow */
                {
                    /* Roll-back temporary index */
                    HumTemp_I2C_slRdBufIndexTmp -= (diffCount - HumTemp_I2C_slOverFlowCount);
                }

                /* Update slave of transferred bytes */
                HumTemp_I2C_slRdBufIndex = HumTemp_I2C_slRdBufIndexTmp;
                ...
                HumTemp_I2C_slStatus &= (uint8) ~HumTemp_I2C_I2C_SSTAT_RD_BUSY;
                HumTemp_I2C_slStatus |= (uint8)  HumTemp_I2C_I2C_SSTAT_RD_CMPLT;
                HumTemp_I2C_state     =  HumTemp_I2C_I2C_FSM_IDLE;
                
            #ifdef HumTemp_I2C_I2C_SLAVE_CMPLT_CALLBACK
                /* Read complete */
                HumTemp_I2C_I2C_SlaveCompleteCallback();
            #endif /* HumTemp_I2C_I2C_SLAVE_CMPLT_CALLBACK */
            }
            ...
            */
            if(HumTemp_I2C_CHECK_INTR_SLAVE_MASKED(HumTemp_I2C_INTR_SLAVE_I2C_WRITE_STOP))
            {
                HumTemp_I2C_ClearSlaveInterruptSource(HumTemp_I2C_INTR_SLAVE_I2C_WRITE_STOP);
                ...
                * I2C_ADDR_MATCH.
                */
                if (0u != (HumTemp_I2C_I2C_CTRL_REG & HumTemp_I2C_I2C_CTRL_S_READY_DATA_ACK))
                {
                    while(0u != HumTemp_I2C_GET_RX_FIFO_ENTRIES)
                    {
                        #if(HumTemp_I2C_CHECK_I2C_ACCEPT_ADDRESS)
                        {
                            if((1u == HumTemp_I2C_GET_RX_FIFO_ENTRIES) &&
                               (HumTemp_I2C_CHECK_INTR_SLAVE_MASKED(HumTemp_I2C_INTR_SLAVE_I2C_ADDR_MATCH)))
                            {
                        ...
                        /* Put data in component buffer */
                        HumTemp_I2C_slWrBufPtr[HumTemp_I2C_slWrBufIndex] = (uint8) HumTemp_I2C_RX_FIFO_RD_REG;
                        HumTemp_I2C_slWrBufIndex++;
                    }

                    HumTemp_I2C_DISABLE_SLAVE_AUTO_DATA;
                }

                if(HumTemp_I2C_CHECK_INTR_RX(HumTemp_I2C_INTR_RX_OVERFLOW))
                {
                    HumTemp_I2C_slStatus |= (uint8) HumTemp_I2C_I2C_SSTAT_WR_OVFL;
                }
                ...
                HumTemp_I2C_slStatus &= (uint8) ~HumTemp_I2C_I2C_SSTAT_WR_BUSY;
                HumTemp_I2C_slStatus |= (uint8)  HumTemp_I2C_I2C_SSTAT_WR_CMPLT;
                HumTemp_I2C_state     =  HumTemp_I2C_I2C_FSM_IDLE;
            ...
            */
        #if (HumTemp_I2C_I2C_CUSTOM_ADDRESS_HANDLER_CONST)
            if (HumTemp_I2C_CHECK_INTR_SLAVE_MASKED(HumTemp_I2C_INTR_SLAVE_I2C_ADDR_MATCH |
        ...
        #endif /* (HumTemp_I2C_I2C_CUSTOM_ADDRESS_HANDLER_CONST) */
            {
                ...
                HumTemp_I2C_ClearI2CExtClkInterruptSource(HumTemp_I2C_INTR_I2C_EC_WAKE_UP);

                #if (HumTemp_I2C_I2C_CUSTOM_ADDRESS_HANDLER)
                {
                    if (NULL != HumTemp_I2C_customAddressHandler)
                    {
                        /* Call custom address handler */
                        response = HumTemp_I2C_customAddressHandler();
                    }
                    else
                    {
                        /* Read address from the RX FIFO. If there is no address underflow triggers but
                        * component does not use that source. */
                        (void) HumTemp_I2C_RX_FIFO_RD_REG;
                        response = HumTemp_I2C_I2C_ACK_ADDR;
                    }
            ...
            #if (HumTemp_I2C_I2C_CUSTOM_ADDRESS_HANDLER_CONST)
                if (response == HumTemp_I2C_I2C_NAK_ADDR)
                {
                #if (!HumTemp_I2C_CY_SCBIP_V0)
                    /* Disable write stop interrupt source as it triggers after address was NACKed. Ticket ID#156094 */
                    HumTemp_I2C_DISABLE_INTR_SLAVE(HumTemp_I2C_INTR_SLAVE_I2C_WRITE_STOP);
                #endif /* (!HumTemp_I2C_CY_SCBIP_V0) */
                    ...
                    HumTemp_I2C_I2C_SLAVE_GENERATE_NACK;
                }
                else
            #endif /* (HumTemp_I2C_I2C_CUSTOM_ADDRESS_HANDLER_CONST) */
                {
                    if(HumTemp_I2C_CHECK_I2C_STATUS(HumTemp_I2C_I2C_STATUS_S_READ))
                    /* TX direction: master reads from slave */
                    {
                        HumTemp_I2C_SetTxInterruptMode(HumTemp_I2C_INTR_TX_EMPTY);

                        /* Set temporary index to address buffer clear from API */
                        HumTemp_I2C_slRdBufIndexTmp = HumTemp_I2C_slRdBufIndex;

                        /* Start master reading */
                        HumTemp_I2C_slStatus |= (uint8) HumTemp_I2C_I2C_SSTAT_RD_BUSY;
                        HumTemp_I2C_state     = HumTemp_I2C_I2C_FSM_SL_RD;
                    }
                    else
                    /* RX direction: master writes into slave */
                    {
                        /* Calculate available buffer size */
                        diffCount = (HumTemp_I2C_slWrBufSize - HumTemp_I2C_slWrBufIndex);
                        ...
                        /* Receive data: into RX FIFO */
                        {
                            if(diffCount == HumTemp_I2C_I2C_FIFO_SIZE)
                            {
                                ...
                                HumTemp_I2C_ENABLE_SLAVE_AUTO_DATA;
                            }
                            else
                            {
                                /* Stretch clock when RX FIFO becomes FULL */
                                HumTemp_I2C_ENABLE_SLAVE_AUTO_DATA_ACK;
                        ...
                        /* Start master reading */
                        HumTemp_I2C_slStatus |= (uint8) HumTemp_I2C_I2C_SSTAT_WR_BUSY;
                        HumTemp_I2C_state     = HumTemp_I2C_I2C_FSM_SL_WR;
                    }
                    ...
                    HumTemp_I2C_ENABLE_INTR_SLAVE(HumTemp_I2C_INTR_SLAVE_I2C_WRITE_STOP);
                #endif /* (!HumTemp_I2C_CY_SCBIP_V0) */

                    /* ACK the address byte */
                    HumTemp_I2C_I2C_SLAVE_GENERATE_ACK;
                }
            }
            ...
            */
            if(HumTemp_I2C_CHECK_INTR_RX_MASKED(HumTemp_I2C_INTR_RX_FULL))
            {
                ...
                diffCount =  HumTemp_I2C_slWrBufSize -
                            (HumTemp_I2C_slWrBufIndex + HumTemp_I2C_I2C_FIFO_SIZE);

                if(diffCount > HumTemp_I2C_I2C_FIFO_SIZE) /* Proceed transaction */
                {
                    diffCount   = HumTemp_I2C_I2C_FIFO_SIZE;
                    endTransfer = 0u;  /* Continue active transfer */
                }
                ...
                {
                    HumTemp_I2C_ENABLE_SLAVE_AUTO_DATA_NACK;
                ...
                HumTemp_I2C_ClearRxInterruptSource(HumTemp_I2C_INTR_RX_FULL);
            }
            ...
            */
            ...
            {
                diffCount = HumTemp_I2C_RX_FIFO_RD_REG;

                if(HumTemp_I2C_slWrBufIndex < HumTemp_I2C_slWrBufSize)
                {
                    HumTemp_I2C_I2C_SLAVE_GENERATE_ACK;

                    /* Put data into component buffer */
                    HumTemp_I2C_slWrBufPtr[HumTemp_I2C_slWrBufIndex] = (uint8) diffCount;
                    HumTemp_I2C_slWrBufIndex++;
                }
                ...
                HumTemp_I2C_ClearRxInterruptSource(HumTemp_I2C_INTR_RX_NOT_EMPTY);
            }
            else
            {
                /* Does nothing */
            ...
            */
            if(HumTemp_I2C_CHECK_INTR_TX_MASKED(HumTemp_I2C_INTR_TX_EMPTY))
            {
                while(HumTemp_I2C_I2C_FIFO_SIZE != HumTemp_I2C_GET_TX_FIFO_ENTRIES)
                {
                    ...
                    */
                    if(HumTemp_I2C_slRdBufIndexTmp < HumTemp_I2C_slRdBufSize)
                    /* Data from buffer */
                    {
                        HumTemp_I2C_TX_FIFO_WR_REG = (uint32) HumTemp_I2C_slRdBufPtr[HumTemp_I2C_slRdBufIndexTmp];
                        HumTemp_I2C_slRdBufIndexTmp++;
                    }
                    else
                    /* Probably Overflow */
                    {
                        HumTemp_I2C_TX_FIFO_WR_REG = HumTemp_I2C_I2C_SLAVE_OVFL_RETURN;

                        if(HumTemp_I2C_slOverFlowCount <= HumTemp_I2C_I2C_TX_OVERFLOW_COUNT)
                        {
                            /* Get counter in range of overflow. */
                            HumTemp_I2C_slOverFlowCount++;
                        }
                    }
                }
...
#ifdef HumTemp_I2C_I2C_ISR_EXIT_CALLBACK
    HumTemp_I2C_I2C_ISR_ExitCallback();
#endif /* HumTemp_I2C_I2C_ISR_EXIT_CALLBACK */
